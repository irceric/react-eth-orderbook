import { Exchange, NormalizedOrder, OrderType, BittrexOrder, PoloniexOrderBook } from '../types';
...
const formatBittrex = (data: {'bid': BittrexOrder[], 'ask': BittrexOrder[], name: string}) => {
  return {
    name: 'bittrex',
...
const placeholderRowData = (data: any, orderType: OrderType) => {
  ...
  sortedPrices.forEach((price) => {
    ...
    data.forEach((exchange: Exchange) => { volPlaceholder[exchange.name] = 0 });
